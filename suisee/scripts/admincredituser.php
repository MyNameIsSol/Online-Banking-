...
include 'db.php';
    if(isset($_POST['credit'])){
        ...
        $_POST['sname'] ? $sname = mysqli_real_escape_string($conn,$_POST['sname']) : $sname ="";
        $bname = $acctname;
        $bnacct = $acctnumber;
        ...
        $_POST['date'] ? $date = $_POST['date'] : $date = date('Y-m-d H:i:s');
        $status ="Successful";

        $transferid="CR".uniqid();
        $transtype = "credit";
        $descrip = "Credit of $".$amount." to Account ".$bnacct." For: ".$comment;
        ...
        if($result_checker > 0){
            while($data = mysqli_fetch_assoc($result)){
                ...
                $sql="INSERT INTO transactions (sendername,senderemail,senderacctnum,bname,bnumber,bbank,description,amount,transactionid,transact_date,bemail,country,rtn,status,initiator_id,transact_type,comment) 
                                    VALUES ('$sname','','','$bname','$bnacct','$bbank','$descrip','$amount','$transferid','$date','$email','$bcountry','$brtn','$status','$uid','$transtype','$comment')";
                ...
                //send transfer successful mail
                          ...
                          <td align="center" style="padding: 0px 10px 0px 10px;">
                            <table border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 600px;">
                              ...
                              <td bgcolor="#ffffff" align="left" style="padding: 10px 10px 20px 10px; color: #666666; font-family: "Poppins", sans-serif; font-size: 16px; font-weight: 400; line-height: 25px;">
                                    <p style="margin:10px 13px; font-size: 14px; text-align:center; color:blue;">We wish to inform you that a Credit transaction occurred on your account with us.</p>
                              </td>
                              </tr>
                              ...
                              <td bgcolor="#ffffff" align="left" style="padding: 10px 10px 40px 10px; color: #111111; font-family: "Poppins", sans-serif; font-size: 14px; font-weight: 400; line-height: 25px;">
                                <h2 style="font-size: 13px; font-weight: 600; margin: 10px 13px; text-align:center; color:red;">Transaction Notification:</h2>
                              </td>
                              </tr>
                              <!-- COPY -->
                              <tr>
                 ...
                 header("Location:../account/admin/credit.php?suc");
                 exit();
            } 
        }
    }else{
        $error = "sorry! Your transaction was terminated. try again...";
		header("Location:../account/admin/credit.php?error=".$error);
		exit();
    }
